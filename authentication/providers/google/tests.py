...
class GoogleTests(OAuth2TestsMixin, TestCase):
    ...
    def test_user_signed_up_signal(self):
        ...
        def on_signed_up(sender, request, user, **kwargs):
            ...
            self.assertEqual(
                sociallogin.account.provider, CustomGoogleProvider.id
            )
    ...
    def test_account_connect(self):
        ...
        self.login(
            self.get_mocked_response(verified_email=True), process="connect"
        )
        # Check if we connected...
        self.assertTrue(
            SocialAccount.objects.filter(
                user=user, provider=CustomGoogleProvider.id
            ).exists()
        )
